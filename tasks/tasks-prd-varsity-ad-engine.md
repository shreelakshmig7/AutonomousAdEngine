...
- `main.py` - Single entry point — one command runs the full pipeline
...
10. main.py                     ← green phase
11. images/image_generator.py   ← v2, last
```
...
- [ ] 2.0 PR2 — Evaluate Module (Judge + Rubrics) — Sunday Goal
  - [ ] 2.1 Write `tests/test_evaluator.py` first (TDD red phase) — all 4 tests with mocked API
  - [ ] 2.2 Confirm all 4 tests FAIL and save results to `tests/results/`
  - [ ] 2.3 Create `evaluate/rubrics.py`:
  ...
  - [ ] 2.4 Create `evaluate/judge.py` — `AdJudge` class with `evaluate_ad()` calling Gemini 1.5 Pro, `build_prompt()` injecting calibration anchors, structured dict returns, full error handling
  - [ ] 2.5 Run tests — confirm all 4 PASS (green), save results
...
- [ ] 4.0 PR4 — Iterate Module + Main Pipeline (50+ Ads)
  ...
  - [ ] 4.5 Create `main.py` — load all data files, run pipeline for all 10+ briefs, write `ads_library.json` + `iteration_log.csv`, validate 50+ ads produced, `rich` live progress
...
- [ ] 5.0 PR5 — v2 Image Generation + Quality Visualization
  ...
  - [ ] 5.4 Verify `output/iteration_log.csv` captures all fields: `brief_id`, `cycle`, all 5 dimension scores, `average_score`, `weakest_dimension`, `status`, `tokens_used`, `cost_usd`
...
- [ ] 6.0 PR6 — Documentation, README & Submission Polish
  - [ ] 6.1 Write `README.md` — one-command cold-start (`python main.py`), prerequisites, `.env` setup, output file descriptions, test run instructions
  - [ ] 6.2 Write `docs/DECISION_LOG.md` — document all 8 key decisions (WHY Flash vs Pro, WHY 5 dimensions, WHY 7.0 threshold, WHY 3 cycles, competitive intelligence methodology, failures, perf-per-token findings, limitations)
